...
export class DurableRunna extends Server<Env> {
  ...
  async getRoughPlan(): Promise<RoughPlan | null> {
    if (this.details) {
      const roughPlan = await buildPlan(
        this.details,
  ...
  buildSystemPrompt() {
    return `You are an agent building a training plan for a user.You will be given a rough plan and you need to converse with the user to build a full plan.You can modify the rough plan at any time the user asks you to. You should always show the user the rough plan before and after you modify it.Use the modify_rough_training_plan tool to modify the rough plan.Once the user is happy you should call the build_full_training_plan tool to build the full plan.Once you have done that you can show the user the full plan and ask them if they would like to make any changes.If they want to make changed you can just re-run the build_full_training_plan tool.These are the details of the user:\n${JSON.stringify(this.details)}\n\nThis is a rough plan we generated earlier:\n${JSON.stringify(this.roughPlan)}`;
  }

  async callAgent(messages: ChatMessage[], _env: Record<string, string>) {
    const tools = {
      modify_rough_training_plan: tool({
        description:
          'Modify the rough training plan. This will be called if the user wants to change the plan in any way.',
        parameters: z.object({
      ...
      build_full_training_plan: tool({
        description:
          'Once the user has verified the rough plan and is happy with the format, sessions, and timings, call this tool to finalise the plan. You will need to make sure you can provide all the details of every session in the plan.',
        parameters: z.object({
  ...
  async onMessage(connection: Connection<unknown>, message: WSMessage) {
    ...
    const parsedMessage = JSON.parse(message as string) as {
      type: string;
      role: string;
      content: string;
    };
...
export async function routePartykitRequest(req: Request, env: Env): Promise<Response | null> {
