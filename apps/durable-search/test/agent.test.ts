...
describe('DurableSearch', () => {
  ...
  describe('upsert', () => {
    ...
    it('should throw an error if vectors length does not match ids length', async () => {
      ...
      await expect(agent.upsert(ids, vectors, attributes)).rejects.toThrow(
        'Vectors array length must match ids length'
      );
    });
    ...
    it('should throw an error if attribute array length does not match ids length', async () => {
      ...
      // Expect the upsert to throw an error
      await expect(agent.upsert(ids, vectors, attributes)).rejects.toThrow(
        'Attribute "title" array length must match ids length'
      );
    });
  });
  ...
  describe('query', () => {
    ...
    it('should filter results based on attribute values using Eq operator', async () => {
      ...
      const queryVector = [0.9, 0.1, 0.1];
      const response = await agent.query(queryVector, 10, DistanceMetric.cosine, [
        'category',
        FilterOperator.Eq,
        'A',
      ]);
