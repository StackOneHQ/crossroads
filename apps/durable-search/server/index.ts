...
export class DurableSearch extends Agent<Env, VectorStoreState> {
  ...
  async query(
    ...
    console.log(
      `Query called with vector of length ${queryVector.length}, topK=${topK}, cursor=${cursor || 'none'}`
    );
    ...
    const documents = new Map(this.state.documents || []);
    console.log(
      `Processing query against ${documents.size} documents starting at index ${startIndex}`
    );
    ...
    console.log(
      `Query returning ${pageResults.length} results, next_cursor=${nextCursor || 'none'}`
    );
  ...
  private matchesFilter(id: string, doc: Document, filter: Filter): boolean {
    ...
    if (Array.isArray(filter) && filter.length === 2) {
      ...
      if (firstElement === FilterOperator.Or && Array.isArray(secondElement)) {
  ...
  private evaluateOperator(
    ...
    switch (operator) {
      ...
      case FilterOperator.Lt:
        ...
        if (typeof filterValue === 'number' && !Number.isNaN(Number(fieldValue))) {
  ...
  async getStats(): Promise<{
    ...
    return {
      ...
      status: 'healthy',
      vectorFormats,
      lastPersisted: this.state.lastPersisted
        ? new Date(this.state.lastPersisted).toISOString()
        : undefined,
      storageUsage,
    };
  }
  ...
  async runMaintenance(_data: { timestamp: number; forcePersist?: boolean }): Promise<void> {
      ...
      case VectorStoreRoutes.Upsert:
        try {
          ...
          try {
            ...
            await stub.upsert(parsedBody.ids, parsedBody.vectors, parsedBody.attributes);
